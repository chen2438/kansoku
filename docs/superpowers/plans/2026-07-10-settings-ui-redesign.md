...
- app/web/src/pages/settings/PrimaryRow.tsx
- app/web/src/pages/settings/RoleRow.tsx
- app/web/src/pages/settings/roleShared.ts
- app/web/src/styles.css
- app/server/package.json、app/pnpm-lock.yaml、app/pnpm-workspace.yaml
- AGENTS.md
...
| 文件 | 动作 | 职责 |
| --- | --- | --- |
| app/web/src/pages/settings/settingsViewModel.ts | 新建 | 纯计算顶部摘要、角色生效状态和问题清单 |
| app/web/src/pages/settings/settingsViewModel.test.ts | 新建 | 验证配置完整度、问题优先级和生效模型 |
| app/web/src/pages/settings/RoleModeControl.tsx | 新建 | 使用原生 radio 实现三态分配控件 |
| app/web/src/pages/settings/SettingsStatusStrip.tsx | 新建 | 展示配置、主模型、Provider 和今日用量摘要 |
| app/web/src/pages/settings/SettingsIssuesPanel.tsx | 新建 | 展示可处理问题与锚点入口 |
| app/web/src/pages/settings/SettingsPage.tsx | 修改 | 数据加载、角色草稿、错误恢复与页面编排 |
| app/web/src/pages/settings/RoleModelsCard.tsx | 修改 | 编排主模型和用途表 |
| app/web/src/pages/settings/PrimaryRow.tsx | 修改 | 改为受控草稿，增加失败回滚与重试 |
| app/web/src/pages/settings/RoleRow.tsx | 修改 | 紧凑行、自定义展开、语义化三态、失败重试 |
| app/web/src/pages/settings/ProviderCredentialsCard.tsx | 修改 | 按 Provider 展示事实状态并原地编辑凭据 |
| app/web/src/pages/settings/types.ts | 修改 | 增加 AiRoles 与视图模型需要的共享类型 |
| app/web/src/pages/settings/saveQueue.ts | 修改 | 错误回调提供可重试的最后用户意图 |
| app/web/src/pages/settings/useSaveQueue.ts | 修改 | 透传失败快照 |
| app/web/src/pages/settings/saveQueue.test.ts | 修改 | 验证失败后的重试快照 |
| app/web/src/styles.css | 修改 | 双栏、状态条、用途表、Provider 与响应式样式 |

### Task 1: 用纯计算层固定页面状态语义

**Files:**
- Create: `app/web/src/pages/settings/settingsViewModel.test.ts`
- Create: `app/web/src/pages/settings/settingsViewModel.ts`
- Modify: `app/web/src/pages/settings/types.ts`
...
```bash
pnpm -C app --filter @trade/web exec vitest run src/pages/settings/settingsViewModel.test.ts
```
...
**Files:**
- Modify: `app/web/src/pages/settings/saveQueue.test.ts`
- Modify: `app/web/src/pages/settings/saveQueue.ts`
- Modify: `app/web/src/pages/settings/useSaveQueue.ts`
...
```bash
pnpm -C app --filter @trade/web exec vitest run src/pages/settings/saveQueue.test.ts
```
...
**Files:**
- Create: `app/web/src/pages/settings/RoleModeControl.tsx`
- Modify: `app/web/src/pages/settings/SettingsPage.tsx`
- Modify: `app/web/src/pages/settings/RoleModelsCard.tsx`
- Modify: `app/web/src/pages/settings/PrimaryRow.tsx`
- Modify: `app/web/src/pages/settings/RoleRow.tsx`
...
```bash
pnpm -C app --filter @trade/web typecheck
```
...
**Files:**
- Create: `app/web/src/pages/settings/SettingsStatusStrip.tsx`
- Create: `app/web/src/pages/settings/SettingsIssuesPanel.tsx`
- Modify: `app/web/src/pages/settings/SettingsPage.tsx`
- Modify: `app/web/src/pages/settings/RoleModelsCard.tsx`
...
- Modify: `app/web/src/pages/settings/PrimaryRow.tsx`
- Modify: `app/web/src/pages/settings/RoleRow.tsx`
- Preserve: `app/web/src/pages/settings/roleShared.ts` 中用户新增的 selectableProviders 逻辑
...
```bash
pnpm -C app --filter @trade/web test
pnpm -C app --filter @trade/web typecheck
```
...
**Files:**
- Verify: `app/web/src/pages/settings/*.tsx`
- Verify: `app/web/src/pages/settings/*.ts`
...
git diff --check
git status --short
git diff -- app/web/src/pages/settings app/web/src/styles.css
```
