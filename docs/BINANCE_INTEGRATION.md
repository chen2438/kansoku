...
首页把长桥持仓和 Binance 持仓分开显示。Binance 当前持仓和已平仓汇总都每分钟刷新，也可以人工立即刷新；已平仓记录按合约汇总近 90 天资金流水。测试网当前持仓支持逐个市价平仓和逐个执行的全部市价平仓，服务端在每次提交前重新读取实时数量，单个失败不会中断其他持仓。净盈亏等于已实现盈亏加交易手续费、资金费、清算调整和手续费返还；转账、赠金及邀请返佣不计入。Binance 资金流水接口只保留近三个月数据，因此这里不是永久历史账本。
