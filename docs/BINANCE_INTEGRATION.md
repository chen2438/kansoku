...
GET  /api/binanceAccount/closed-position-history
GET  /api/binanceAccount/open-orders
POST /api/binanceAccount/testnet/orders
POST /api/binanceAccount/testnet/positions/close
POST /api/binanceAccount/testnet/positions/close-all
POST /api/binanceAccount/testnet/orders/cancel
```

四个写接口只接受人工确认后的测试网开仓、单个或全部整仓市价平仓、撤单。开仓参数包括方向、初始保证金、杠杆倍数，以及可选的止盈价和止损价。服务端会再次检查当前凭证必须属于测试网；主网开仓和平仓不会仅靠页面隐藏，而是直接返回拒绝。
...
首页取消原来的“看盘”区域，把 Binance 持仓放在左侧主栏，长桥持仓保留在右侧。Binance 当前持仓和历史平仓都每分钟刷新，也可以人工立即刷新；当前持仓同时显示交易来源、未实现盈亏、已计入持仓的手续费/资金费和净浮盈亏。费用由 Binance 盈亏平衡价与开仓价反推，因此不包含尚未成交的未来平仓手续费；缺少有效盈亏平衡价时费用显示为未知。历史平仓默认按合约汇总近 90 天累计净盈亏和胜率，点击合约后展开细目；胜率按一分钟合并后的组计算，净盈亏大于零算盈利，小于零算亏损，等于零不计入胜负。细目精确到秒并按平仓时间从新到旧排列，同一合约相邻两次平仓不超过 60 秒时合并为一组，盈亏和费用相加，显示时间取组内最后一次平仓。服务端通过对应的 Binance 成交记录补充做多/做空方向，一分钟组内两种方向都有时显示“多空混合”，成交方向无法取得时显示“方向未知”。已查到的方向会缓存在当前服务进程，避免每分钟重复消耗请求权重。自动开仓来源和人工下单来源保存在本机加密的 Binance 账户记录中，当前持仓和历史细目显示“成交额榜”“涨幅榜”“跌幅榜”或“手动”；本功能上线前的旧订单以及无法匹配的记录显示“来源未知”，一分钟组内包含多个来源时显示“混合来源”。交易手续费优先按同一 tradeId 归入对应平仓，没有 tradeId 的资金费等调整归入同合约下一笔平仓。测试网当前持仓支持逐个市价平仓和逐个执行的全部市价平仓，服务端在每次提交前重新读取实时数量，单个失败不会中断其他持仓。历史净盈亏不计转账、赠金及邀请返佣。Binance 资金流水接口只保留近三个月数据，因此这里不是永久历史账本。
...
首页的 `AI 分析 Binance Top 20` 会按 Binance 24 小时 USDT 成交额实时选取交易中的永续合约，并以 2 路并发运行 Analyst。自动下单另提供“涨幅 Top 10”和“跌幅 Top 10”：只包含正在交易的 USDT 永续合约，按 Binance 24 小时涨跌幅从高到低或从低到高选取。批次状态仅保存在当前进程内，服务重启后清空；每项只有实际生成新图表才算完成。

其下方的 `AI 分析 Binance Top 20 并下单` 只允许在已连接 Binance 期货测试网时启动，并需要用户先核对一次风险确认弹窗。每个标的生成新分析后立即处理，不检查入场是否已经触发：做多或做空会以 20 USDT 初始保证金、20 倍杠杆市价开仓，AI 止损价作为止损，目标1作为止盈；观望不下单。方向明确但缺少有效止损价或目标1时跳过下单并显示失败原因。每项分别记录分析状态、下单状态、开仓订单号和保护单错误。

自动下单批次在分析前先读取一次 Binance 当前持仓。Top 20 中已经持有合约仓位的标的直接跳过 AI 分析和下单，避免浪费分析请求。没有仓位的标的完成分析后，服务端仍会在开仓前重新查询该标的的实时持仓、普通挂单和条件单；任一存在就跳过，不加仓、不平仓、不反手。该规则只作用于首页自动批次，设置页人工开仓不受影响。自动订单带本批次稳定的客户端订单编号，方便识别重复请求和后续排查；程序不会自动撤销用户手动创建的订单。

首页可启动“每小时自动分析并下单”。启动前可以选择成交额 Top 20、涨幅 Top 10 或跌幅 Top 10；启动后立即运行第一轮，之后每小时重新读取所选榜单和当前持仓。任务可以设置结束时间，也可以持续运行并由用户点击停止。自动任务运行在本机服务端，关闭浏览器页面不影响运行，但关闭或重启本机服务会清空任务；任务配置不写入凭证库或交易日志。停止自动任务只会取消下一轮，正在执行的一轮会安全完成。
...
- Binance 账号支持读取余额、当前持仓、近 90 天已平仓净盈亏和普通挂单；开仓只支持测试网人工确认的市价开仓。
...
- 首页新增 Binance 当前持仓和近 90 天逐笔平仓净盈亏；记录精确到秒并按最新平仓优先显示，同一合约一分钟内的相邻平仓合并显示，净盈亏计入交易手续费、资金费、清算调整和手续费返还。
- 首页取消“看盘”区域，Binance 持仓移到左侧；历史平仓默认按合约汇总累计净盈亏，点击后展开保留一分钟合并规则的逐笔细目。
- Binance 当前持仓分开显示未实现盈亏、已计手续费/资金费和净浮盈亏；历史平仓细目补充做多、做空、多空混合或方向未知。
- Binance 当前持仓和历史细目显示成交额榜、涨幅榜、跌幅榜或手动来源；历史合约汇总按一分钟组合计算胜率。
- 首页 Binance 持仓新增人工刷新、单个市价平仓和全部市价平仓；全部平仓逐个执行并汇总成功与失败结果。
- Binance 当前持仓改为按盈亏平衡价显示净浮盈亏；未来平仓手续费只在实际成交后计入历史净盈亏。
- 首页新增 `AI 分析 Binance Top 20 并下单`，分析完成一项就立即按固定 20 USDT 保证金、20 倍杠杆在测试网执行；做多/做空使用市价开仓、AI 止损价和目标1，观望跳过。
- 自动下单新增 Binance 24 小时涨幅 Top 10、跌幅 Top 10；每小时自动任务可在三个榜单之间选择。
...
- 自动下单批次在调用 AI 前先跳过已有仓位的标的；首页可按每小时持续运行或设置结束时间，并支持人工停止。
